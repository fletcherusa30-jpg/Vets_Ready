...
export function generateMarkdownExport(wizardState: WizardState): string {
  ...
  md += `---\n\n`;
  md += `*Document generated by VetsReady on ${timestamp}*\n`;
...
export function downloadMarkdown(wizardState: WizardState, filename?: string): void {
  ...
  a.href = url;
  a.download = filename || `vetsready-claim-strategy-${Date.now()}.md`;
...
export async function generatePDFExport(wizardState: WizardState): Promise<void> {
  try {
    ...
    const doc = new jsPDF();
    doc.text('VetsReady Claim Strategy Export', 10, 10);
    ...
    doc.save('vetsready-claim-strategy.pdf');
  } catch (err) {
...
export function downloadJSON(wizardState: WizardState, filename?: string): void {
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = filename || `vetsready-export-${Date.now()}.json`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
