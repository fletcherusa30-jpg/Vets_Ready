...
export const BenefitsPage: React.FC = () => {
  ...
  const loadBenefits = async () => {
    ...
    try {
      if (activeTab === 'federal') {
        const res = await fetch('/api/vetsready/benefits/federal')
        const data = await res.json()
        setFederalBenefits(data || [])
      } else {
        const res = await fetch(`/api/vetsready/benefits/state/${selectedState}`)
        const data = await res.json()
  ...
  return (
    <div className="vetsready-benefits">
      {/* Hero */}
      <section className="bg-gradient-to-r from-green-900 to-green-700 text-white py-16">
        <div className="max-w-7xl mx-auto px-4">
          <h1 className="text-4xl font-bold mb-4">Vets Ready Benefits Navigator</h1>
