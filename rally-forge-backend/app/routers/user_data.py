...
async def export_user_data(
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        # Add JSON data
        zip_file.writestr(
            f"rallyforge_data_export_{current_user.id}.json",
            json_data
        )

        # Add README
        readme = f"""Rally Forge DATA EXPORT

Exported: {datetime.utcnow().isoformat()}
User ID: {current_user.id}
Email: {current_user.email}

This archive contains all your personal data stored by Rally Forge.

Files:
- rallyforge_data_export_{current_user.id}.json: Your data in JSON format
...
To exercise your rights, contact: privacy@rallyforge.com

For more information, see our Privacy Policy:
https://rallyforge.com/privacy
"""
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=rallyforge_data_export_{current_user.id}.zip"
        }
    )
...
async def delete_account(
    ...
    return {
        ...
        "recovery_info": "You can recover your account within 30 days by contacting support@rallyforge.com"
    }
