"""
Rally Forge - Main FastAPI Application
...
from app.routers import (
    ...
    dd214_scanner,  # DD-214 OCR scanner with diagnostics
...
app.include_router(scanner_api.router)  # Scanner API - /api/scan/* endpoints
app.include_router(scanner_api.scanner_router)  # Scanner legacy API - /api/scanner/* endpoints
app.include_router(dd214.router)  # DD-214 document extraction
app.include_router(dd214_scanner.router)  # DD-214 OCR scanner with diagnostics
...
async def shutdown_event():
    """Cleanup on shutdown"""
    logger.info("Shutting down Rally Forge backend")
...
async def root():
    """API root endpoint"""
    return {
        "message": "Rally Forge Backend API - The Ultimate Veteran-First Platform",
        "version": "1.0.0",
        "docs": "/docs",
        "redoc": "/redoc",
        "features": {
