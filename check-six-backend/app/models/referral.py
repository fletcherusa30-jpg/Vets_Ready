...
class Referral(Base):
    """Referral tracking"""
    __tablename__ = "rallyforge_referrals"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    referrer_user_id = Column(UUID(as_uuid=True), ForeignKey("rallyforge_users.id"), nullable=False)
    referred_user_id = Column(UUID(as_uuid=True), ForeignKey("rallyforge_users.id"), nullable=True)
...
class ReferralReward(Base):
    ...
    __tablename__ = "rallyforge_referral_rewards"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(UUID(as_uuid=True), ForeignKey("rallyforge_users.id"), nullable=False)
    referral_id = Column(UUID(as_uuid=True), ForeignKey("rallyforge_referrals.id"), nullable=False)
