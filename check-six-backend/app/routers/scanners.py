...
async def upload_str_from_app(
    filename: str = Query(..., description="Filename located in C:/Dev/Rally Forge/App"),
    volume: Optional[str] = None,
    background_tasks: BackgroundTasks = None
):
    """
    ...
    This is handy for testing when sample documents live in `C:/Dev/Rally Forge/App`.
    """
    try:
        allowed_extensions = ['.pdf', '.tiff', '.tif', '.jpg', '.jpeg', '.png', '.heic']
...
async def run_project_scanner(background_tasks: BackgroundTasks):
    ...
    This executes scripts/Scan-Android.ps1 (but for rallyforge project)
    """
    try:
        job_id = str(uuid.uuid4())
...
async def run_diagnostics():
    ...
    try:
        ...
        required_dirs = [
            ...
            LOGS_DIR,
            REPORTS_DIR,
            PROJECT_ROOT / "rally-forge-frontend",
            PROJECT_ROOT / "rally-forge-backend",
        ]
