...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('vetsready_users.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_table(
        'vetsready_job_posts',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('employer_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('vetsready_employer_accounts.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('veteran_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('vetsready_users.id')),
        sa.Column('business_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('vetsready_business_listings.id')),
        sa.Column('employer_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('vetsready_employer_accounts.id')),
    ...
    op.create_table(
        'vetsready_payments',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('invoice_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('vetsready_invoices.id', ondelete='SET NULL')),
        sa.Column('customer_type', sa.String(20), nullable=False),
        sa.Column('customer_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('amount', sa.Numeric(10, 2), nullable=False),
        sa.Column('payment_method', sa.String(50)),
    ...
    # Create indexes
    op.create_index('idx_veteran_subscriptions_user', 'vetsready_veteran_subscriptions', ['user_id'])
    op.create_index('idx_veteran_subscriptions_tier', 'vetsready_veteran_subscriptions', ['tier'])
    op.create_index('idx_veteran_subscriptions_status', 'vetsready_veteran_subscriptions', ['status'])

    op.create_index('idx_employer_accounts_tier', 'vetsready_employer_accounts', ['tier'])
    op.create_index('idx_employer_accounts_status', 'vetsready_employer_accounts', ['status'])
    op.create_index('idx_employer_accounts_email', 'vetsready_employer_accounts', ['contact_email'])

    op.create_index('idx_job_posts_employer', 'vetsready_job_posts', ['employer_id'])
    op.create_index('idx_job_posts_status', 'vetsready_job_posts', ['status'])
    op.create_index('idx_job_posts_featured', 'vetsready_job_posts', ['featured'])

    op.create_index('idx_business_listings_tier', 'vetsready_business_listings', ['tier'])
    op.create_index('idx_business_listings_status', 'vetsready_business_listings', ['status'])
    op.create_index('idx_business_listings_category', 'vetsready_business_listings', ['category'])

    op.create_index('idx_leads_veteran', 'vetsready_leads', ['veteran_id'])
    op.create_index('idx_leads_business', 'vetsready_leads', ['business_id'])
    op.create_index('idx_leads_employer', 'vetsready_leads', ['employer_id'])
    op.create_index('idx_leads_status', 'vetsready_leads', ['status'])

    op.create_index('idx_vso_partners_status', 'vetsready_vso_partners', ['status'])
    op.create_index('idx_vso_partners_email', 'vetsready_vso_partners', ['contact_email'])

    op.create_index('idx_invoices_customer', 'vetsready_invoices', ['customer_type', 'customer_id'])
    op.create_index('idx_invoices_status', 'vetsready_invoices', ['status'])
    op.create_index('idx_invoices_number', 'vetsready_invoices', ['invoice_number'])

    op.create_index('idx_payments_invoice', 'vetsready_payments', ['invoice_id'])
    op.create_index('idx_payments_customer', 'vetsready_payments', ['customer_type', 'customer_id'])
    op.create_index('idx_payments_status', 'vetsready_payments', ['status'])
...
def downgrade() -> None:
    ...
    # Drop tables
    op.drop_table('vetsready_payments')
    op.drop_table('vetsready_invoices')
    op.drop_table('vetsready_vso_partners')
    op.drop_table('vetsready_leads')
    op.drop_table('vetsready_business_listings')
    op.drop_table('vetsready_job_posts')
    op.drop_table('vetsready_employer_accounts')
    op.drop_table('vetsready_veteran_subscriptions')
