...
// VA Payment Rates by Year (Auto-updates annually on January 1st)
const VA_PAYMENT_RATES_BY_YEAR: { [key: number]: { [key: number]: { alone: number; withSpouse: number; perChild: number } } } = {
  2024: {
  ...
  2025: {
    // 2025 rates (8.16% COLA increase) - effective January 1, 2025
    10: { alone: 185.32, withSpouse: 185.32, perChild: 0 },
    20: { alone: 365.77, withSpouse: 365.77, perChild: 0 },
    30: { alone: 566.04, withSpouse: 619.43, perChild: 29.42 },
    40: { alone: 816.26, withSpouse: 896.00, perChild: 41.28 },
    50: { alone: 1161.44, withSpouse: 1266.68, perChild: 58.38 },
    60: { alone: 1471.60, withSpouse: 1597.60, perChild: 73.56 },
    70: { alone: 1855.11, withSpouse: 2011.29, perChild: 92.82 },
    80: { alone: 2157.05, withSpouse: 2336.15, perChild: 107.27 },
    90: { alone: 2422.44, withSpouse: 2624.00, perChild: 120.80 },
    100: { alone: 4038.04, withSpouse: 4265.10, perChild: 131.87 }
  }
};

// SMC Rates by Year (also auto-update annually)
const SMC_RATES_BY_YEAR: { [key: number]: any } = {
  ...
  2025: {
    K: 145.16, // Aid & Attendance - 8.07% increase
    L: 5146.56, // Loss of use of one limb - 8.06% increase
    L_half: 6080.80, // Loss of use of both feet or hands
    M: 6729.43, // Loss of use of both feet or hands + other
    N: 7380.79, // Higher level
    O: 9415.42, // Highest level
    R1: 4270.72, // Housebound (with 100% rating)
  }
};

// Get current year rates automatically
const getCurrentYear = (): number => new Date().getFullYear();
const getVAPaymentRates = (): { [key: number]: { alone: number; withSpouse: number; perChild: number } } => {
...
export const calculateDisabilityCompensation = (profile: VeteranProfile): number => {
  ...
  const currentRates = getVAPaymentRates();
