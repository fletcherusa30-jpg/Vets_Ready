...
const CompleteClaimWizard: React.FC = () => {
   ...
   */
  useEffect(() => {
    localStorage.setItem('rallyforgeWizardData', JSON.stringify(wizardData));
  }, [wizardData]);
   ...
   */
  useEffect(() => {
    const saved = localStorage.getItem('rallyforgeWizardData');
    if (saved) {
      try {
  ...
  const exportSummary = () => {
    ...
    a.href = url;
    a.download = `rallyforge-ClaimPrep-${new Date().toISOString().split('T')[0]}.html`;
    a.click();
...
<html>
<head>
  <title>rallyforge Claim Preparation Summary</title>
  <style>
...
<body>
  <h1>🎖️ rallyforge Claim Preparation Summary</h1>
  ...
  <p style="font-size: 12px; color: #6b7280;">
    Generated by rallyforge.com - Educational Tool Only<br>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <select
          id="state"
          name="state"
          value={wizardData.basics.state}
          onChange={(e) => setWizardData(prev => ({
            ...prev,
            basics: { ...prev.basics, state: e.target.value }
          }))}
          className="w-full px-4 py-3 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
        >
        ...
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
          Your state is required to determine eligible state-specific benefits
        </p>
      </div>
    </div>
  );
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {/* SMC Qualifiers - Show when SMC is checked */}
        {wizardData.disability.hasSMC && (
          <div className="ml-8 mt-4 p-4 border-l-2 border-blue-500 bg-blue-50 dark:bg-blue-900/20 space-y-3">
            <p className="text-sm text-gray-700 dark:text-gray-300 font-semibold">
              SMC Qualifiers - Select all that apply:
            </p>

            <label htmlFor="smc-aid-attendance" className="flex items-center cursor-pointer">
              <input
                type="checkbox"
                id="smc-aid-attendance"
                name="smc-aid-attendance"
                checked={wizardData.disability.smcQualifiers.hasAidAndAttendance}
                onChange={(e) => setWizardData(prev => ({
                  ...prev,
                  disability: {
                    ...prev.disability,
                    smcQualifiers: {
                      ...prev.disability.smcQualifiers,
                      hasAidAndAttendance: e.target.checked
                    }
                  }
                }))}
                className="h-5 w-5 text-blue-600 rounded"
              />
              <span className="ml-3 text-gray-900 dark:text-white text-sm">
                Aid & Attendance (need help with daily living activities)
              </span>
            </label>

            <label htmlFor="smc-housebound" className="flex items-center cursor-pointer">
              <input
                type="checkbox"
                id="smc-housebound"
                name="smc-housebound"
                checked={wizardData.disability.smcQualifiers.isHousebound}
                onChange={(e) => setWizardData(prev => ({
                  ...prev,
                  disability: {
                    ...prev.disability,
              ...
              />
              <span className="ml-3 text-gray-900 dark:text-white text-sm">
                Permanently Housebound (unable to leave home without assistance)
              </span>
            </label>

            <label htmlFor="smc-loss-of-limb" className="flex items-center cursor-pointer">
              <input
                type="checkbox"
                id="smc-loss-of-limb"
                name="smc-loss-of-limb"
                checked={wizardData.disability.smcQualifiers.hasLossOfUseOfLimb}
                onChange={(e) => setWizardData(prev => ({
                  ...prev,
                  disability: {
                    ...prev.disability,
                    smcQualifiers: {
                      ...prev.disability.smcQualifiers,
                      hasLossOfUseOfLimb: e.target.checked
                    }
                  }
                }))}
                className="h-5 w-5 text-blue-600 rounded"
              />
              <span className="ml-3 text-gray-900 dark:text-white text-sm">
                Loss of Use or Loss of a Limb (anatomical loss or loss of use)
              </span>
            </label>

            <label htmlFor="smc-blindness" className="flex items-center cursor-pointer">
              <input
                type="checkbox"
                id="smc-blindness"
                name="smc-blindness"
                checked={wizardData.disability.smcQualifiers.hasBlindness}
                onChange={(e) => setWizardData(prev => ({
                  ...prev,
                  disability: {
                    ...prev.disability,
              ...
              />
              <span className="ml-3 text-gray-900 dark:text-white text-sm">
                Blindness (total blindness in both eyes or loss of use of one eye)
              </span>
            </label>

            <p className="text-xs text-gray-600 dark:text-gray-400 mt-3">
              <strong>Note:</strong> SMC requires both a VA rating AND at least one of these qualifiers. More information available at VA.gov
            </p>
          </div>
        )}
      </div>
